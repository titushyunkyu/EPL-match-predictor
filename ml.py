...
# Function to predict matches for a specified week
def predict_week(week_number):
    # Calculate the start and end dates for the specified week
    first_week_start = future_matches['Date'].min()
    week_start = first_week_start + pd.Timedelta(days=(week_number - 1) * 7)
    week_end = week_start + pd.Timedelta(days=7)
    
    # Filter for the specified week of future matches
    week_matches = future_matches[(future_matches['Date'] >= week_start) & (future_matches['Date'] <= week_end)].copy()
    
    # Make predictions using the trained model
    future_preds = rf.predict(week_matches[future_predictors])
    
    # Add predictions to the future matches DataFrame
    week_matches['Prediction'] = future_preds
    
    # Ensure the 'Prediction' column is of type int before mapping
    week_matches['Prediction'] = week_matches['Prediction'].astype(int)
    week_matches['Prediction'] = week_matches['Prediction'].map({2: 'Win', 1: 'Draw', 0: 'Loss'})
    
    # Display the predictions
    print(week_matches[['Date', 'Team', 'Opponent', 'Prediction']])

# Interactive part: Prompt the user for the week number
while True:
    week_number = input("Enter the week number for which you want predictions (or 'exit' to quit): ")
    if week_number.lower() == 'exit':
        break
